...
export const useProductsStore = defineStore('products', {
    ...
    actions: {
        add(product: Omit<Product, 'id' | 'created_at'>) {
            ...
            persist(this.list);
            Notify.create({ type: 'positive', message: 'Producto agregado', icon: 'check' });
            return true;
        },
        update(id: string, patch: Partial<Omit<Product, 'id' | 'created_at'>>) {
            ...
            persist(this.list);
            Notify.create({ type: 'positive', message: 'Producto actualizado', icon: 'check' });
            return true;
        },
        remove(id: string) {
            ...
            if (this.list.length !== before) {
                persist(this.list);
                Notify.create({ type: 'info', message: 'Producto eliminado', icon: 'delete' });
                return true;
            }
            return false;
        },
        clearAll() {
            this.list = [];
            persist(this.list);
            Notify.create({ type: 'info', message: 'Lista de productos vacía', icon: 'delete' });
        },
